function [problem,guess] = TwoDPE_GTCCU
%OrbitRaising
%
...
InternalDynamics=@TwoDPE_GTCCU_Dynamics_Internal;
SimDynamics=@TwoDPE_GTCCU_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_TwoDPE_GTCCU;
...
problem.time.tf_min=90;     
problem.time.tf_max=90; 
guess.tf=90;
...
% disp(init_psi_e);
% pause(2);
problem.states.x0=[xp0  yp0  psip0  xe0  ye0  psie0  -1 -2 -1];
 
% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[xp0  yp0  psip0  xe0  ye0  psie0   -20   -20  -20 ]; 
problem.states.x0u=[xp0  yp0  psip0  xe0  ye0  psie0    30     30    30];


% State bounds. xl=< x <=xu
bnd = 100;
problem.states.xl=[-bnd -bnd  -pi -bnd -bnd -pi   -bnd   -bnd  -bnd];
problem.states.xu=[bnd   bnd   pi  bnd  bnd  pi    bnd     bnd    bnd];


% State error bounds
problem.states.xErrorTol_local=[1e-2 1e-2 1e-2  1e-2 1e-2 1e-2 1e-2 1e-2 1e-2];
problem.states.xErrorTol_integral=[1e-2 1e-2 1e-2 1e-2 1e-2 1e-2 1e-2 1e-2 1e-2];

% State constraint error bounds
problem.states.xConstraintTol=[1e-2 1e-2 1e-2 1e-2 1e-2 1e-2 1e-2 1e-2 1e-2];


% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[-20 -20  -20 -20 -20 -20   -20   -20  -20]; 
problem.states.xfu=[20   20   20  20  20  20    30     30    30];

% Guess the state trajectories with [x0 xf]
% guess.time=[0 3.32];
guess.states(:,1)=[0.5  0];
guess.states(:,2)=[1.5 2.5];
guess.states(:,3)=[-1   0];
guess.states(:,4)=[-1   0];
guess.states(:,5)=[-1   0];
guess.states(:,6)=[-1   0];
...
% Input bounds
problem.inputs.ul=[-pi];
problem.inputs.uu=[pi];

% Bounds on first control action
problem.inputs.u0l=[-pi];
problem.inputs.u0u=[pi]; 

% Input constraint error bounds
problem.inputs.uConstraintTol=[0.01];

% Guess the input sequences with [u0 uf]
guess.inputs(:,1)=[-pi pi];
...
problem.constraints.bl=[0 0 0 ];
problem.constraints.bu=[0 0 0 ];
problem.constraints.bTol=[0.01 0.01 0.01 ];


% store the necessary problem parameters used in the functions
%   problem.data = []; % [lb]
problem.data.K_E = 100 ;
problem.data.K_P = 100;
problem.data.v_p = 0.3;
problem.data.v_e = 0.2;
...
boundaryCost = -0.5.*((xf(1)-xf(4)).^2+(xf(2)-xf(5)).^2);
