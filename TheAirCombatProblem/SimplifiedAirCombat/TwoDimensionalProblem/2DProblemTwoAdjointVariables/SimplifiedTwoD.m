...
problem.states.xfl=[1e-5   -10   -10   0  -10  -10  1e-5   -10   -10]; 
problem.states.xfu=[10     10    10    0   10   10  10     10    10];
...
% Input bounds
problem.inputs.ul=[-pi/3];
problem.inputs.uu=[pi/3];

% Bounds on first control action
problem.inputs.u0l=[-pi/3];
problem.inputs.u0u=[pi/3]; 
...
r_shoot = 0.1;
bc=[xf(8)-xf(2)-r_shoot;xf(3)-xf(9)];
