...
problem.states.x0=[1    0  2.5 ...
                   2    1   0.01 ...
                   1    1  2.5 ];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[1    0   2.5 ...
                    2    1   0.01 ...
                    1    1    2.5 ]; 
                
problem.states.x0u=[1    0   2.5 ...
                    5    5    5 ...
                    1    1    2.5 ]; 
...
% Input bounds
problem.inputs.ul=[-pi/6];
problem.inputs.uu=[pi/6];

% Bounds on first control action
problem.inputs.u0l=[-pi/6];
problem.inputs.u0u=[pi/6]; 

% Input constraint error bounds
problem.inputs.uConstraintTol=[0.01];

% Guess the input sequences with [u0 uf]
guess.inputs(:,1)=[-pi/6  pi/6];
...
problem.constraints.bTol=[1e-2 1e-2 1e-2];
...
boundaryCost = 0.5.*((xf(2)-xf(8)).^2+(xf(3)-xf(9)).^2);
