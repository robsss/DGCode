function [problem,guess] = SimplifiedTwoD
%OrbitRaising
%
...
InternalDynamics=@SimplifiedTwoD_Dynamics_Internal;
SimDynamics=@SimplifiedTwoD_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_SimplifiedTwoD;
...
problem.time.tf_min=1.5;     
problem.time.tf_max=2.5; 
guess.tf=2;
...
problem.states.x0=[v_p0    x_p0    y_p0 ...
                   1.67     1     0 ...
                   v_e0    x_e0    y_e0 ];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[v_p0      x_p0       y_p0 ...
                    -cosbnd  -cosbnd    -cosbnd ...
                    v_e0      x_e0       y_e0 ]; 
                
problem.states.x0u=[v_p0      x_p0      y_p0 ...
                    cosbnd    cosbnd    cosbnd ...
                    v_e0      x_e0      y_e0 ]; 
...
problem.states.xl=[-10  -10 -10 -10   -10  -10  -10   -10  -10];
problem.states.xu=[10    10  10  10    10   10   10    10  10];
...
problem.states.xfl=[1e-5   -10   -10   -10  -10  -10  1e-5   -10   -10]; 
problem.states.xfu=[10     10    10    10   10   10  10     10    10];

% Guess the state trajectories with [x0 xf]
% guess.time=[0 3.32];
guess.states(:,1)=[1     2];
guess.states(:,2)=[0     4];
guess.states(:,3)=[2.5   0];
guess.states(:,4)=[-1    0];
guess.states(:,5)=[-1    0];
guess.states(:,6)=[-1    0];
guess.states(:,7)=[1     3];
