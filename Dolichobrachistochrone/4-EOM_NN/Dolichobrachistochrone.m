function [problem,guess] = Dolichobrachistochrone
%OrbitRaising
%
...
InternalDynamics=@Dolichobrachistochrone_Dynamics_Internal;
SimDynamics=@Dolichobrachistochrone_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_Dolichobrachistochrone;
...
problem.states.x0=[x_0 y_0 1 lambda_0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[x_0 y_0 -10 -10]; 
problem.states.x0u=[x_0 y_0  10 10]; 



% State bounds. xl=< x <=xu
problem.states.xl=[0  0 -10 -10];
problem.states.xu=[20 20 10  10];


% State error bounds
problem.states.xErrorTol_local=[1e-2 1e-2 1e-2 1e-2];
problem.states.xErrorTol_integral=[1e-2 1e-2 1e-2 1e-2];

% State constraint error bounds
problem.states.xConstraintTol=[1e-2 1e-2 1e-2 1e-2];


% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[0   0  -10 0]; 
problem.states.xfu=[10 10  10  0];

% Guess the state trajectories with [x0 xf]
% guess.time=[0 3.32];
guess.states(:,1)=[1.5 0];
